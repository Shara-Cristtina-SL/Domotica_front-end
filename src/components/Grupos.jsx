...
export default function Grupos() {
  ...
  if (errorGrupos) setToast({ message: "Erro ao carregar grupos", variant: "error" });
  if (errorDispositivos) setToast({ message: "Erro ao carregar dispositivos", variant: "error" });
  ...
  async function handleCadastrar() {
    ...
    if (novoDispositivos.length === 0) {
      setToast({ message: "Selecione pelo menos um dispositivo", variant: "error" });
      return;
    }
    ...
    try {
      ...
      setToast({ message: "Grupo cadastrado com sucesso!", variant: "success" });
      setNovoNome("");
      setNovoDispositivos([]);
      await refetchGrupos();
    } catch {
  ...
  async function salvarEdicao() {
    ...
    setLoading(true);
    try {
      await editarGrupo(editandoId, { nome: editandoNome, dispositivos: editandoDispositivos });
  ...
  return (
    <div className="max-w-md mx-auto p-4">
      ...
      <div className="flex flex-col mb-4 space-y-2">
        ...
        <div className="max-h-48 overflow-auto border rounded p-2 mb-4">
          {dispositivos?.map((dispositivo) => (
            <label key={dispositivo.idDispositivo} className="flex items-center space-x-2">
              <input
                type="checkbox"
                ...
                onChange={(e) => {
                  ...
                  if (e.target.checked) setNovoDispositivos([...novoDispositivos, id]);
                  else setNovoDispositivos(novoDispositivos.filter((i) => i !== id));
                }}
              />
      ...
      <ul>
        {grupos?.map((grupo) => (
          <li key={grupo.idGrupo} className="mb-2 flex flex-col space-y-2 border p-3 rounded">
            {editandoId === grupo.idGrupo ? (
                  ...
                  {dispositivos?.map((dispositivo) => (
                    <label key={dispositivo.idDispositivo} className="flex items-center space-x-2">
                      <input
                        type="checkbox"
                        value={dispositivo.idDispositivo}
                        checked={editandoDispositivos.includes(dispositivo.idDispositivo)}
                        onChange={(e) => {
                          const id = dispositivo.idDispositivo;
                          if (e.target.checked) setEditandoDispositivos([...editandoDispositivos, id]);
                          else setEditandoDispositivos(editandoDispositivos.filter((i) => i !== id));
                        }}
                      />
                      <span>{dispositivo.nome}</span>
                    </label>
              ...
              <>
                <div className="flex justify-between items-center">
                  ...
                  <div>
                    <Button label="Editar" onClick={() => iniciarEdicao(grupo)} variant="edit" />
                    <Button label="Excluir" onClick={() => handleExcluir(grupo.idGrupo)} variant="delete" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside">
                    {grupo.dispositivos.length > 0
                      ? grupo.dispositivos.map((d) => <li key={d.idDispositivo}>{d.nome}</li>)
                      : <li>Nenhum dispositivo</li>}
                  </ul>
                </div>

                <div className="flex justify-between items-center mt-2">
                  <Button label="Ligar Grupo" onClick={() => handleLigar(grupo.idGrupo)} variant="default" />
                  <Button label="Desligar Grupo" onClick={() => handleDesligar(grupo.idGrupo)} variant="default" />
                </div>
              </>
            )}
          </li>
